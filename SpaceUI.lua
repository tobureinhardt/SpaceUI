...
do
    Assets.Dashboard.NewTab = function(data)
        ...
        -- Spawn Position: Mặc định nằm tại tâm Dashboard (MainFrame)
        if InitInfo and InitInfo.Objects and InitInfo.Objects.MainFrame then
            tab.Objects.ActualTab.Position = InitInfo.Objects.MainFrame.Position
        else
            tab.Objects.ActualTab.Position = UDim2.fromScale(0.5, 0.5)
        end

        -- Spawn Size: Mặc định lấy đúng 80% kích thước thực tế của Dashboard
        local parentScreen = SpaceUI.Background and SpaceUI.Background.Objects and SpaceUI.Background.Objects.MainScreenGui
        local parentSize = (parentScreen and parentScreen.AbsoluteSize) or (UserCamera and UserCamera.ViewportSize) or Vector2.new(1920, 1080)
        local dashSize = (InitInfo and InitInfo.Objects and InitInfo.Objects.MainFrame and InitInfo.Objects.MainFrame.AbsoluteSize)
        if dashSize and dashSize.X > 50 and parentSize.X > 50 then
            tab.Objects.ActualTab.Size = UDim2.fromScale((dashSize.X * 0.8) / parentSize.X, (dashSize.Y * 0.8) / parentSize.Y)
        else
            local cfgW = SpaceUI.Config.UI.Size and SpaceUI.Config.UI.Size.X or 0.7
            local cfgH = SpaceUI.Config.UI.Size and SpaceUI.Config.UI.Size.Y or 0.9
            local scale = SpaceUI.Config.UI.Scale or 0.5
            tab.Objects.ActualTab.Size = UDim2.fromScale(cfgW * scale * 0.8, cfgH * scale * 0.8)
        end
