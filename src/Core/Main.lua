return function(Assets, SpaceUI, Services)
    ...
    Assets.Main.SetupTopbarButton = function()
        if SpaceUI.TopbarIcon then
            return SpaceUI.TopbarIcon
        end

        local baseUrl = SpaceUI.TopbarPlusUrl or "https://raw.githubusercontent.com/Therealtobu/Topbar-Plus-For-Executor/main/"
        local Icon = loadstring(game:HttpGet(baseUrl .. "init.lua"))()
        ...
        topbarIcon.selected:Connect(function()
            if SpaceUI.TopbarSyncing then return end
    ...
    Assets.Main.Uninject = function()
        ...
        if SpaceUI.TopbarIcon and SpaceUI.TopbarIcon.destroy then SpaceUI.TopbarIcon:destroy() end
    ...
    Assets.Main.ToggleVisibility = function(visible)
        do
            if visible == nil and SpaceUI.Background and SpaceUI.Background.Objects and SpaceUI.Background.Objects.MainFrame then
                visible = not SpaceUI.Background.Objects.MainFrame.Visible
            end
            ...
            if not SpaceUI.Config.UI.Anim then
                ...
                if SpaceUI.TopbarIcon then
                    SpaceUI.TopbarSyncing = true
                    if visible and not SpaceUI.TopbarIcon.isSelected then
                        SpaceUI.TopbarIcon:select()
                    elseif not visible and SpaceUI.TopbarIcon.isSelected then
                        SpaceUI.TopbarIcon:deselect()
                    end
                    SpaceUI.TopbarSyncing = false
                end
                return
            end
